			...
			if (sender instanceof Player) {
				...
				if ((args.length == 0)) {
					...
					sender.sendMessage(ChatColor.RED
							+ "Wrong args. Try one of these:");
					sender.sendMessage(ChatColor.GREEN + "/disco create "
							+ ChatColor.GRAY + "<name>");
					sender.sendMessage(ChatColor.GREEN + "/disco finish");
					sender.sendMessage(ChatColor.GREEN + "/disco reset");
					sender.sendMessage(ChatColor.GREEN + "/disco remove "
							+ ChatColor.GRAY + "<name>");
						...
						if (discoMain.using.contains(playr)) {
							...
							if (args.length >= 2) {
								...
								if (!sender.isOp()) {
									...
									sender.sendMessage("Disco " + disconame
											+ " created.");
						...
						if (discoMain.using.contains(playr)) {
							...
							if (!sender.isOp()) {
								...
								sender.sendMessage(ChatColor.AQUA
										+ "Avaiable disco's: "
										+ ChatColor.GREEN
										+ discoPreferences.discoList.toString());
								return true;
							}
						}
					}
